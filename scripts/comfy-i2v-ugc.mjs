/**
 * NORA — Image-to-Video UGC (LTX-Video 2.3 I2V con audio + RTX upscale)
 * Anima una imagen existente en video vía ComfyUI remoto (PC-2), merge audio, upscale RTX x2, sube a Supabase Storage.
 *
 * Uso: node comfy-i2v-ugc.mjs [--once] [--id=123] [--max=1] [--strength=1.0]
 ...
 *   --strength=F Adherencia a imagen fuente (default: 1.0)
 *
 * La creatividad debe tener:
 *   - prompt: texto para LTX-Video 2.3 (describe movimiento/animación)
 *   - url: URL del audio WAV en Supabase Storage
 *   - link_ren_2: URL de la imagen fuente
 *   - origen: "i2v"
...
const STRENGTH = strengthArg ? parseFloat(strengthArg.split('=')[1]) : 1.0;
...
// =============================================================
// Workflow builder — LTX-Video 2.3 Image-to-Video con audio
// =============================================================
...
function buildWorkflow(prompt, audioFilename, imageFilename, seed, duration, id) {
  ...
  // === IMAGE (fuente) ===
  w["150"] = { inputs: { image: imageFilename, upload: "image" }, class_type: "LoadImage" };
  ...
  // === I2V CONDITIONING (reemplaza EmptyLTXVLatentVideo + LTXVConditioning) ===
  w["250"] = {
    inputs: {
      positive: ["200", 0], negative: ["201", 0],
      vae: ["102", 0], image: ["150", 0],
  ...
  w["320"] = { inputs: { video_latent: ["250", 2], audio_latent: ["314", 0] }, class_type: "LTXVConcatAVLatent" };

  // === SAMPLING ===
  w["400"] = { inputs: { noise_seed: seed }, class_type: "RandomNoise" };
  w["401"] = { inputs: { model: ["101", 0], positive: ["250", 0], negative: ["250", 1], cfg: 1.0 }, class_type: "CFGGuider" };
  ...
  w["411"] = { inputs: { samples: ["410", 0], filename_prefix: `LTX-2.3/i2v_${id}_latent` }, class_type: "SaveLatent" };
...
async function getPendingCreatividades() {
  let url = `${SUPA}/rest/v1/creatividades?estado=eq.para_ejecucion&origen=eq.i2v&prompt=not.is.null&url=not.is.null&link_ren_2=not.is.null&select=id,prompt,marca,url,link_ren_2,concepto&order=id.asc`;
  if (onlyId) {
    url = `${SUPA}/rest/v1/creatividades?id=eq.${onlyId}&origen=eq.i2v&select=id,prompt,marca,url,link_ren_2,concepto`;
  }
  const r = await fetch(url, { headers: { 'Authorization': `Bearer ${KEY}`, 'apikey': KEY } });
  const data = await r.json();
  return data.filter(c => c.prompt && c.prompt.trim().length > 0 && c.url && c.url.trim().length > 0 && c.link_ren_2 && c.link_ren_2.trim().length > 0);
}
...
async function downloadImage(imageUrl) {
  log(`  📥 Descargando imagen fuente: ${imageUrl.split('/').pop()}`);
...
async function uploadImageToComfyUI(imageBuffer, filename) {
  ...
  if (!r.ok) {
    ...
    throw new Error(`Image upload to ComfyUI failed: ${r.status} ${err}`);
  }
  const data = await r.json();
  log(`  🖼️ Imagen subida a ComfyUI: ${data.name}`);
  return data.name;
}
...
async function queuePrompt(workflow) {
  const r = await fetch(`${COMFY}/prompt`, {
    ...
    body: JSON.stringify({ prompt: workflow, client_id: 'nora-ltx23-i2v' })
  });
  const data = await r.json();
...
async function uploadToSupabase(videoBuffer, marca) {
  ...
  const name = `${safeMarca}_i2v_${Date.now()}.mp4`;
...
async function processOne(creatividad) {
  const { id, prompt, marca, url: audioUrl, link_ren_2: imageUrl } = creatividad;
  log(`▶ #${id} [${marca}] — Generando video I2V (strength=${STRENGTH})...`);

  // 0. Mark as processing to prevent duplicate pickup
  await markAsProcessing(id);

  // 1. Download source image from Supabase
  const imageBuffer = await downloadImage(imageUrl);
  log(`  🖼️ Imagen descargada (${(imageBuffer.length / 1024).toFixed(0)} KB)`);

  // 2. Download audio from Supabase
  const rawAudio = await downloadAudio(audioUrl);
  log(`  📥 Audio descargado (${(rawAudio.length / 1024).toFixed(0)} KB)`);

  // 3. Pad audio with 1s silence at the end (for smile transition to pack de cierre)
  const audioBuffer = padAudioWithSilence(rawAudio, 1.0);

  // 4. Get padded audio duration
  ...
  // 5. Upload source image to ComfyUI input folder
  const ext = imageUrl.includes('.jpg') || imageUrl.includes('.jpeg') ? '.jpg' : '.png';
  const imageFilename = `nora_i2v_${id}_${Date.now()}${ext}`;
  const comfyImageName = await uploadImageToComfyUI(imageBuffer, imageFilename);

  // 6. Upload padded audio to ComfyUI input folder
  const audioFilename = `nora_i2v_${id}_${Date.now()}.wav`;
  const comfyAudioName = await uploadAudioToComfyUI(audioBuffer, audioFilename);

  // 7. Build workflow and queue
  const seed = randomSeed();
  const workflow = buildWorkflow(prompt, comfyAudioName, comfyImageName, seed, duration, id);
  const promptId = await queuePrompt(workflow);
  log(`  ⏳ Queued (${promptId.substring(0, 8)}...) seed=${seed} duration=${duration.toFixed(1)}s`);

  // 8. Wait for video completion
  const videoInfo = await waitForCompletion(promptId);
  log(`  ✅ Video generado: ${videoInfo.filename}`);

  // 9. Download from ComfyUI
  ...
  // 10. Merge padded Cartesia audio into video (includes trailing silence)
  const mergedBuffer = mergeAudio(rawBuffer, audioBuffer);

  // 11. RTX Video Super Resolution (x2 upscale)
  log(`  🚀 RTX upscale x2 — subiendo video a ComfyUI...`);
  const rtxInputName = await uploadVideoToComfyUI(mergedBuffer, `nora_i2v_${id}_base_${Date.now()}.mp4`);
  ...
  // 12. Upload to Supabase Storage (1152×2048 — postprod Remotion escala a 1080×1920)
  const videoUrl = await uploadToSupabase(upscaledBuffer, marca);
  log(`  ☁️ Subido: ${videoUrl.split('/').pop()}`);

  // 13. Update creatividad
...
async function run() {
  log('🎬 NORA Image-to-Video UGC — Iniciando');
