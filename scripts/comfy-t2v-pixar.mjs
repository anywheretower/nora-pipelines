/**
 * NORA — Text-to-Video PIXAR (LTX-Video 2.3 con audio + RTX upscale)
 * Genera video estilo animación 3D Pixar vía ComfyUI remoto (PC-2).
 * Mismo flujo que UGC (audio Cartesia + lip-sync + RTX upscale) pero con estilo Pixar.
 *
 * Uso: node comfy-t2v-pixar.mjs [--once] [--id=123] [--max=1] [--singlepass] [--base] [--raw]
 ...
 *   --singlepass  Desactiva multipass (por defecto usa 3 pasos de sampling)
 *   --base        Solo video base, sin RTX upscale x2
 *   --raw         No transforma el prompt (usa tal cual de Supabase)
 *
...
const multipass = !args.includes('--singlepass'); // Pixar: multipass ON por defecto (3D surfaces need refinement)
...
function buildWorkflow(prompt, audioFilename, seed, duration, id, enableMultipass = false) {
  ...
  const NEGATIVE = 'live action, realistic, photographic, camera equipment, tripod visible, text, watermark, blurry, out of focus, soft focus, ugly, closed mouth, silent, not speaking, uncanny valley, low poly, flat shading, golden hour, warm color cast, slow motion';

  const w = {};

  // === MODELS (LTX 2.3) ===
  w["100"] = { inputs: { unet_name: "ltx-2-3-22b-dev-Q4_K_M.gguf" }, class_type: "UnetLoaderGGUF" };
  w["101"] = { inputs: { lora_name: "ltx-2.3-22b-distilled-lora-384.safetensors", strength_model: 0.3, model: ["100", 0] }, class_type: "LoraLoaderModelOnly" };
  ...
  w["401"] = { inputs: { model: ["101", 0], positive: ["202", 0], negative: ["202", 1], cfg: 1.3 }, class_type: "CFGGuider" };
  ...
  w["411"] = { inputs: { samples: ["410", 0], filename_prefix: `LTX-2.3/pixar_${id}_latent` }, class_type: "SaveLatent" };
  ...
  w["800"] = {
    inputs: {
      ...
      filename_prefix: "nora-t2v-pixar",
...
async function getPendingCreatividades() {
  let url = `${SUPA}/rest/v1/creatividades?estado=eq.para_ejecucion&origen=eq.pixar&prompt=not.is.null&url=not.is.null&select=id,prompt,marca,url,concepto&order=id.asc`;
  if (onlyId) {
    url = `${SUPA}/rest/v1/creatividades?id=eq.${onlyId}&origen=eq.pixar&select=id,prompt,marca,url,concepto`;
  }
...
async function queuePrompt(workflow) {
  const r = await fetch(`${COMFY}/prompt`, {
    ...
    body: JSON.stringify({ prompt: workflow, client_id: 'nora-ltx23-pixar' })
  });
  const data = await r.json();
...
async function uploadToSupabase(videoBuffer, marca) {
  ...
  const name = `${safeMarca}_pixar_${Date.now()}.mp4`;
...
async function processOne(creatividad) {
  ...
  log(`▶ #${id} [${marca}] — Generando video Pixar...`);

  // Pixar style: envolver prompt con guía de animación 3D
  if (!rawMode && prompt) {
    prompt = `3D Pixar-style animated character, sharp detailed CGI render, bright natural daylight, smooth stylized features, expressive big eyes with frequent blinking, lively exaggerated animated movements, high detail textures and surfaces. ${prompt} Rendered in crisp high-definition 3D animation style, clean detailed surfaces, vivid saturated colors, 4K sharpness, Pixar Dreamworks quality.`;
    log('  🎬 Prompt transformado a estilo Pixar');
  }
  ...
  const audioFilename = `nora_pixar_${id}_${Date.now()}.wav`;
  ...
  log(`  ⏳ Queued (${promptId.substring(0, 8)}...) seed=${seed} duration=${duration.toFixed(1)}s${multipass ? ' [MULTIPASS 3 pasos]' : ' [SINGLEPASS]'}`);
  ...
  if (baseOnly) {
    ...
    const rtxInputName = await uploadVideoToComfyUI(mergedBuffer, `nora_pixar_${id}_base_${Date.now()}.mp4`);
...
async function run() {
  log(`🎬 NORA Text-to-Video PIXAR — Iniciando${multipass ? ' [MULTIPASS: 3 pasos de sampling]' : ' [SINGLEPASS]'}`);
