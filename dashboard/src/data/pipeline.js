...
export const pipelines = [
  ...
  // ============================================================
  // Pipeline 7: Image-to-Video UGC (LTX-Video 2.3 I2V + Cartesia TTS)
  // ============================================================
  {
    id: 'img2video-ugc',
    title: 'Image-to-Video · UGC',
    subtitle: 'Video animado desde imagen + voz Cartesia TTS + LTX-Video 2.3 I2V',
    command: '/nora-video-i2v',
    status: 'activo',
    ...
    executionBlocks: [
      ...
      {
        executor: 'skill',
        label: 'Skill: nora-video-i2v',
        phases: ['lectura', 'procesamiento'],
        handoff: 'ejecuta script via Bash',
      },
      {
        executor: 'script',
        label: 'Script: comfy-i2v-ugc.mjs',
        phases: ['ejecucion', 'entrega'],
        handoff: 'creatividad para_revision → espera aprobación',
      },
      {
    ...
    phases: {
      activador: {
        ...
        stateIn: null,
        stateOut: null,
        description: 'El usuario invoca /nora-video-i2v indicando la marca y la imagen fuente (ID de creatividad o URL directa).',
        supabaseFields: { reads: {}, writes: {} },
        steps: [
          {
            label: 'Instrucción del usuario',
            resource: { type: 'usuario', name: 'Terminal Claude Code' },
            description: 'El usuario indica marca, imagen fuente (ID de creatividad aprobada o URL), ángulo emocional y dirección creativa.',
            details: [
              'marca — Nombre exacto en Supabase (obligatorio)',
              'imagen — ID de creatividad aprobada o URL directa (obligatorio)',
              'ángulo — Qué dolor/beneficio comunicar',
              'movimiento — Tipo de animación deseada (sutil, expresivo, etc.)',
            ],
          },
          {
            label: 'Cargar variables de entorno',
      ...
      lectura: {
        title: 'Identidad de marca',
        executor: 'skill',
        executorDetail: 'nora-video-i2v',
        stateIn: null,
        stateOut: null,
        description: 'Se lee la ficha completa de la marca para generar un libreto y prompt de movimiento coherentes.',
        supabaseFields: {
          reads: {
            marcas: ['ficha', 'arquetipo', 'buyer_persona', 'notas_generales', 'contenido_prohibido'],
          },
        ...
        steps: [
          ...
          {
            label: 'Obtener imagen fuente',
            resource: { type: 'supabase', name: 'READ creatividades', op: 'READ' },
            description: 'Si el usuario indica un ID de creatividad, se lee link_ren_1 como imagen fuente. Si provee URL directa, se usa tal cual.',
            details: [
              'ID → READ creatividades.link_ren_1 WHERE id = {id}',
              'URL → se usa directamente sin query',
            ],
            filter: 'id = {imagen_id} (solo si se indica ID)',
          },
        ],
      },

      procesamiento: {
        title: 'Libreto + Voz + Prompt movimiento + INSERT',
        executor: 'skill',
        executorDetail: 'nora-video-i2v',
        stateIn: null,
        stateOut: 'para_ejecucion',
        description: '4 sub-pasos: libreto (~15-20 palabras), voz Cartesia TTS (WAV), prompt de movimiento I2V, INSERT creatividad.',
        supabaseFields: {
          reads: {},
          writes: {
            storage: ['{marca}_i2v_audio_{timestamp}.wav → bucket creatividades (audio Cartesia)'],
            creatividades: {
              insert: [
                'marca', 'estado → para_ejecucion', 'origen → i2v', 'condicion → null',
                'prompt → texto movimiento LTX en inglés', 'url → audio WAV en Storage',
                'link_ren_2 → URL imagen fuente',
                'concepto', 'slogan_headline', 'copy',
              ],
            },
          },
        ...
        steps: [
          {
            label: 'Escribir libreto',
            resource: { type: 'skill', name: 'nora-video-i2v' },
            description: 'Texto de 15-20 palabras (~4-5 segundos). Hook → Beneficio → CTA. Adaptado a la imagen fuente.',
          },
          {
            label: 'Generar voz Cartesia TTS',
            resource: { type: 'api', name: 'Cartesia Sonic 3' },
            ...
            details: [
              'Modelo: sonic-3',
              'Voces: 17 voces chilenas clonadas',
              'Speed: slow (0.9-1.05 según marca)',
              'Formato: WAV 44100Hz mono',
          ...
          {
            label: 'Construir prompt de movimiento I2V',
            resource: { type: 'skill', name: 'nora-prompt-ltxvideo' },
            description: 'Párrafo en inglés que describe movimiento/animación, NO la escena completa (la imagen ya la define). 3-5 oraciones.',
            details: [
              'Describe: movimiento labial, gestos sutiles, cámara, ambiente',
              'NO re-describir: apariencia, ropa, fondo (ya en la imagen)',
              'Cierre: personaje deja de hablar y sonríe a cámara',
              'Más corto que T2V (3-5 oraciones vs 4-8)',
            ],
          },
          {
            label: 'Insertar creatividad',
            resource: { type: 'supabase', name: 'INSERT creatividades', op: 'INSERT' },
            description: 'Creatividad con prompt de movimiento, audio URL, imagen fuente y campos de copy.',
            details: [
              'estado: para_ejecucion',
              'origen: i2v',
              'prompt: texto de movimiento LTX en inglés',
              'url: audio WAV en Supabase Storage',
              'link_ren_2: URL imagen fuente',
              'concepto, slogan_headline, copy',
            ],
      ...
      ejecucion: {
        title: 'ComfyUI remoto — LTX-Video 2.3 I2V + RTX Upscale',
        executor: 'script',
        executorDetail: 'comfy-i2v-ugc.mjs',
        stateIn: 'para_ejecucion',
        stateOut: null,
        description: 'El script marca en_proceso, descarga imagen + audio, los sube a ComfyUI, envía el workflow LTX 2.3 I2V (LTXVImgToVideo), espera el video, mergea audio, aplica RTX upscale x2 y lo descarga. Si falla, marca como error.',
        supabaseFields: {
          reads: {
            creatividades: ['id', 'prompt', 'marca', 'url', 'link_ren_2', 'concepto'],
          },
          writes: {
            creatividades: {
              update_on_pickup: ['estado → en_proceso'],
          ...
          filters: ['estado = para_ejecucion', 'origen = i2v', 'prompt NOT NULL', 'url NOT NULL', 'link_ren_2 NOT NULL'],
        },
        steps: [
          {
            label: 'Descargar imagen fuente',
            resource: { type: 'script', name: 'comfy-i2v-ugc.mjs → fetch imagen' },
            description: 'Descarga imagen PNG/JPG desde Supabase Storage (link_ren_2).',
          },
          {
            label: 'Descargar audio + pad silencio',
            resource: { type: 'script', name: 'comfy-i2v-ugc.mjs → fetch audio + ffmpeg apad' },
            description: 'Descarga WAV desde Supabase y añade 1s de silencio al final (ffmpeg apad).',
          },
          {
            label: 'Subir imagen + audio a ComfyUI',
            resource: { type: 'script', name: 'comfy-i2v-ugc.mjs → POST /upload/image x2' },
            description: 'Sube imagen y audio a la carpeta input de ComfyUI via /upload/image.',
          },
          {
            label: 'Enviar workflow LTX 2.3 I2V',
            resource: { type: 'script', name: 'comfy-i2v-ugc.mjs → POST /prompt' },
            description: 'Workflow LTX 2.3 Image-to-Video con audio lip-sync nativo + SaveLatent.',
            details: [
              'Modelo: ltx-2-3-22b-dev-Q4_K_M.gguf',
              'CLIP: gemma-3-12b-it-IQ4_XS.gguf + ltx-2.3_text_projection',
              'LoRA distilled (0.5) — 1 sola LoRA',
              'I2V: LoadImage → LTXVImgToVideo (strength 1.0)',
              'Sampler: euler_ancestral_cfg_pp, 8 steps, CFG 1.0',
              'Resolución: 576×1024 (9:16), 24fps',
              'Audio: MelBandRoFormer → LTXVAudioVAEEncode (mask=0)',
              'SaveLatent: i2v_{id}_latent en PC-2 (backup)',
            ],
          },
          {
            label: 'Esperar generación LTX',
            resource: { type: 'script', name: 'comfy-i2v-ugc.mjs → poll /history' },
            description: 'Polling cada 5s hasta output en /history. Timeout 15 min.',
            details: [
              'Busca outputs["800"].gifs[0] (VHS_VideoCombine)',
              'Si status_str = error → falla',
          ...
          {
            label: 'Descargar video + merge audio',
            resource: { type: 'script', name: 'comfy-i2v-ugc.mjs → GET /view + ffmpeg' },
            description: 'Descarga MP4 (576×1024) y mergea audio Cartesia original.',
          },
          {
            label: 'RTX Video SuperResolution x2',
            resource: { type: 'script', name: 'comfy-i2v-ugc.mjs → POST /upload/image + POST /prompt' },
            description: 'Sube video mergeado a ComfyUI y ejecuta workflow RTX upscale (NVIDIA TensorRT).',
            details: [
              'Workflow: workflows/rtx-video-upscale.json (5 nodos)',
              'LoadVideo → GetVideoComponents → RTXVideoSuperResolution → CreateVideo → SaveVideo',
          ...
          {
            label: 'Descargar video upscaleado',
            resource: { type: 'script', name: 'comfy-i2v-ugc.mjs → GET /view' },
            description: 'Descarga MP4 upscaleado (1152×2048) desde ComfyUI.',
          },
        ],
        meta: [
          ...
          { icon: '⚠️', label: 'Límite', value: 'Máx 1 video por corrida (VRAM leak)' },
          { icon: '🔎', label: 'Upscale', value: 'RTX TensorRT x2 ULTRA inline (576→1152)' },
          { icon: '🖼️', label: 'I2V', value: 'LTXVImgToVideo — strength 1.0 (ajustable vía --strength)' },
        ],
      },
      ...
      entrega: {
        ...
        supabaseFields: {
          reads: {},
          writes: {
            storage: ['{marca}_i2v_{timestamp}.mp4 → bucket creatividades (video upscaleado 1152×2048)'],
            creatividades: {
              update: ['link_ren_1 → URL pública video upscaleado', 'link_ren_2 → URL pública video upscaleado', 'estado → base_lista', 'condicion → para_revision'],
            },
          },
        ...
        steps: [
          {
            ...
            resource: { type: 'supabase', name: 'INSERT storage', op: 'INSERT' },
            description: 'Upload del video upscaleado (1152×2048) al bucket "creatividades".',
            details: ['Nombre: creatividades/{marca}_i2v_{timestamp}.mp4'],
          },
          {
            label: 'Actualizar creatividad',
            resource: { type: 'supabase', name: 'UPDATE creatividades', op: 'UPDATE' },
      ...
      postprod: {
        ...
        stateIn: 'aprobado',
        stateOut: null,
        description: 'Tras aprobación humana: descarga video upscaleado (1152×2048), Remotion escala a 1080p. Transcribe audio (Whisper CUDA), render dual (9:16 + 4:5) con subtítulos karaoke y pack de cierre. Mismo script que T2V UGC.',
        manual: true,
        supabaseFields: {
          reads: {
            creatividades: ['id', 'marca', 'link_ren_1', 'link_ren_2', 'url', 'prompt', 'concepto', 'copy', 'slogan_headline'],
        ...
        steps: [
          ...
          },
          {
            ...
            resource: { type: 'script', name: 'postprod-ugc.mjs → generateCompositionTsx + remotion render' },
            description: 'Genera TSX con subtítulos hardcodeados. Render 1080×1920 (9:16) + 1080×1350 (4:5).',
          },
        ],
        meta: [
          { icon: '⚙️', label: 'Hardware', value: 'PC-2: RTX 5080 16GB (Whisper CUDA + Remotion)' },
          { icon: '⏱️', label: 'Tiempo', value: '~3-5 min (whisper ~30s + 2 renders ~2min c/u)' },
          { icon: '🎬', label: 'Output', value: '2 videos: 1080×1920 (9:16) + 1080×1350 (4:5)' },
          { icon: '♻️', label: 'Reutiliza', value: 'Mismo postprod-ugc.mjs que T2V UGC — sin cambios' },
        ],
      },
      ...
      entrega_final: {
        ...
        supabaseFields: {
          ...
          writes: {
            storage: [
              '{marca}_i2v_{timestamp}_916.mp4 → bucket creatividades (9:16 stories)',
              '{marca}_i2v_{timestamp}_45.mp4 → bucket creatividades (4:5 feed)',
            ],
            creatividades: {
              insert: [
                'marca', 'estado → ejecutado', 'origen → i2v', 'condicion → para_revision',
                'link_ren_1 → video 4:5 (feed)', 'link_ren_2 → video 9:16 (stories)',
                'prompt, concepto, copy, slogan_headline → heredados de creatividad base',
              ],
            },
        ...
        steps: [
          ...
          },
          {
            label: 'Upload video 9:16',
            resource: { type: 'supabase', name: 'UPSERT storage', op: 'UPSERT' },
            description: 'creatividades/{marca}_i2v_{id}_916.mp4 → link_ren_2 (PUT x-upsert)',
          },
          {
            label: 'Upload video 4:5',
            resource: { type: 'supabase', name: 'UPSERT storage', op: 'UPSERT' },
            description: 'creatividades/{marca}_i2v_{id}_45.mp4 → link_ren_1 (PUT x-upsert)',
          },
          {
            label: 'Crear creatividad final',
            resource: { type: 'supabase', name: 'INSERT creatividades', op: 'INSERT' },
